...
##################################################################
##Question 3 Solution
#Note: The Link provided in the assignment document is not an Online link. Downloaded a similar dataset,
#Age column was not having NA Values. So inserted 300 NA values using Random Distribution. 
#Inserting Random NA values in Age column of titan dataframe.
ind<- which(titan$age %in% sample(titan$age,15))
titan$age[ind] <- NA
class(titan$age) <- "numeric"
hist(titan$age)
curve(dnorm(x, mean=mean(titan$age), sd=sd(titan$age)), add=TRUE, col="red") 
################################
##Question 1 Solution
setwd("D://ACADGILD//VImal//VISUALIZATION AND PLOTTING//CODING//INPUT")
titan<-read.table(file = 'titanic.txt', sep = '\t', header = TRUE)
